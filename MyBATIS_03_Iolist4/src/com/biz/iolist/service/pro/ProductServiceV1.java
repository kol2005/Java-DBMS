...
public class ProductServiceV1 {
	...
	public ProductServiceV1() {
		...
		this.proDao = DBConnection.getsqlSessionFactory().openSession(true)
				.getMapper(ProductDao.class);
	}
	
	/*
	 * protected 현재 클래스를 상속받은 곳에서는 
	 * viewPDetail() method 를 사용할수 있지만
	 * 그렇지 않능 곳에서는 접근 금지
	 * 
	...
	protected ProductDTO viewPDetail(String strPCode) {
		// 입력한 상품코드를 다시 조회
				...
				/*
				 * 과세칼럼에 값이 null 일 경우는 "과세"를 기본값으로 표시
	...
	//	}
		 ...
		 * Integer.valueOf(strIPrice)에서 Exception이 발생할 것이다
		 * try{} 로 감싸진 코드에서 Exception이 발생을 하면
		 * 나머지 코드는 무시되고 catch()로 jump 한다
		 * 
		 * 이런 기능을 역 이용하여
		 ...
		 * 원래 DB table에서 가져온 값이 그대로 유지 될 것이고
		 * 
		 * 값을 입력했다면
		 * 새로운 값으로 변경되어 있을 것이다
		 */
